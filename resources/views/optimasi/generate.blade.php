...
<div class="container">
    @if(session('result') || isset($result))
        ...
        <div class="alert alert-success">
            ...
            <p>Waktu Generate: {{ $resultData['execution_time'] }} detik</p>
            <p>Fitness Akhir: {{ $resultData['fitness'] }}</p>
        </div>
    ...
    <!-- Form yang sudah ada tetap sama -->
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Generate Optimasi</h6>
        </div>
        <div class="card-body">
            <form action="{{ route('optimasi.generate.store') }}" method="post">
                @csrf
                <div class="form-row m-2 form-optimasi">
                    <div class="col">
                        <label for="kromosom">Kromosom</label>
                        <input type="number" name="kromosom" class="form-control" value="10" placeholder="Kromosom" required>
                    </div>
                    <div class="col">
                        <label for="max_gen">Maksimal Generasi</label>
                        <input type="number" name="max_gen" class="form-control" value="10" placeholder="Maksimal Generasi" required>
                    </div>
                    <div class="col">
                        <label for="titik_awal">Titik Awal</label>
                        <select class="custom-select" name="titik_awal" id="inputGroupSelect01" required>
                ...
                <div class="form-row m-2 form-optimasi">
                    <div class="col">
                        <input type="text" value="0.8" name="crossover_rate" class="form-control" placeholder="Crossover Rate" required>
                    </div>
                    <div class="col">
                        <input type="text" value="0.1" name="mutation_rate" class="form-control" placeholder="Mutation Rate" required>
                    </div>
                </div>
                <div class="form-row m-2 form-optimasi">
                    <div class="col text-center">
                        <button type="submit" class="btn btn-primary mb-2 btn-optimasi">Submit</button>
                    </div>
                </div>
            </form>
        </div>
...
let currentDirectionsResult = null; // Tambahkan variable untuk menyimpan hasil directions
...
function initMap() {
    ...
    directionsRenderer = new google.maps.DirectionsRenderer({
        ...
        suppressPolylines: false, // Ubah menjadi false
        preserveViewport: true,
        panel: document.getElementById('directions-panel') // Tambahkan ini
    });
    ...
    directionsRenderer.setMap(map); // Pindahkan ini ke sini

    // Tambahkan kontrol untuk toggle polyline dan directions
    ...
    @if(isset($result))
        ...
        // Calculate dan tampilkan directions
...
async function calculateAndDisplayRoute(coordinates) {
    ...
    try {
        ...
        currentDirectionsResult = result; // Simpan hasil directions
        if (isDirectionsVisible) {
